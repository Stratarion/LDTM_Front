...
import AddReviewForm from '@/components/features/garden/AddReviewForm'

interface GardenReviewsProps {
  organizationId: string
}

export default function GardenReviews({ 
  organizationId, 
}: GardenReviewsProps) {
  const { user } = useAuth()
  ...
  const loadReviews = async () => {
    try {
      const reviews = await ReviewsService.getSchoolReviews(organizationId)
      setReviews(reviews)
      // Проверяем, есть ли отзыв текущего пользователя
      if (user) {
        const userReview = reviews.find((review: Review) => review.creater_id === user.id)
    ...
    <div className="space-y-6">
      <div className="border-b border-gray-100 pb-6">
        {hasUserReview ? (
          ...
          <>
            ...
            <AddReviewForm gardenId={organizationId} onReviewAdded={loadReviews} />
          </>
        )}
      </div>
      ...
      {reviews && reviews.length > 0 ? (
        ...
        <div className="text-center text-gray-500 py-8">
          Пока нет отзывов. Будьте первым, кто оставит отзыв об этом детском саде!
        </div>
      )}
    </div>
  )
